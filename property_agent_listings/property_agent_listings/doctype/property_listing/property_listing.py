...
class PropertyListing(WebsiteGenerator):
    ...
    def validate(self):
        ...
        if self.google_drive_folder:
            self.google_drive_link = f'<a href="{self.google_drive_folder}" target="_blank">Open Google Drive</a>'
        else:
        ...
        if self.perumahan_cluster:
            cluster_name = frappe.db.get_value("Perumahan Cluster", self.perumahan_cluster, "perumahan_cluster_name")
            if cluster_name:
        ...
        kecamatan_name = getattr(self, "kecamatan_name", None)
